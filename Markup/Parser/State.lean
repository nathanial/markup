/-
  Markup/Parser/State.lean - Parser state and monad definition
-/
...
/-- Parser state tracking position in HTML input -/
structure ParserState where
  input : String
  pos : Nat := 0
  line : Nat := 1
  column : Nat := 1
  tagStack : List String := []  -- Stack of open tags for validation
  deriving Repr
...
def tryString (expected : String) : Parser Bool := do
  ...
  if ahead == expected then
    -- Consume the string, updating line/column
